...
for BED_CHUNK in $(ls -v kordofan-giraffe_1mb_scaffolds.bed.*); do
  # append command to a joblist
  echo "python genome_fragments.py $FASTAS -b $BED_CHUNK -c -n 0.2 -s 450000" >> final_gfs.jobs
done
# run multiple instances of 'genome_fragments.py' in parallel
cat final_gfs.jobs | parallel -k -j 50
...
for GF in $(ls -v *.fa); do
  # append command to a joblist
  echo "sed '/^[^>]/ s/[^N]//gi; /^\s*$/d' $GF | python check_gfs.py $GF 450000 51 > ${GF/.fa/.percent_n}" >> check_gfs_450kb.jobs
done
# run multiple instances of 'check_gfs.py' in parallel
cat check_gfs_450kp.jobs | parallel -j 20
